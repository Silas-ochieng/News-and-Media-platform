...
def resend_verification(request):
    ...
    try:
        logger.info(f"Attempting to resend verification email to {request.user.email}")
        send_mail(
        ...
        logger.error(f"Failed to resend verification email to {request.user.email}: {str(e)}")
        messages.error(
            request,
