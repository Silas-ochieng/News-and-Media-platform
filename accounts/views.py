...
def register_view(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            # Send verification email without threading for better error handling
            try:
                ...
                # Still log the user in even if email fails
                login(request, user)
                messages.warning(request, 'Registration successful! However, we could not send the verification email. You can resend it from your profile.')
                return redirect('accounts:verify_email_required')
    else:
